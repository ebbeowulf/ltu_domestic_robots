...
rclcpp::Publisher<CameraInfo>::SharedPtr ir_info_pub;

// Global camera info messages
CameraInfo color_info_msg;
CameraInfo depth_info_msg;
CameraInfo ir_info_msg;

// One-shot subscriptions - no thermal because it doesn't have a camera info topic
rclcpp::Subscription<CameraInfo>::SharedPtr cinfo_sub;
rclcpp::Subscription<CameraInfo>::SharedPtr dinfo_sub;
rclcpp::Subscription<CameraInfo>::SharedPtr irinfo_sub;

typedef message_filters::sync_policies::ApproximateTime<Image, Image, Image, Image> MySyncPolicy;
...
void irInfoCallback(const CameraInfo::SharedPtr msg)
{
  RCLCPP_INFO(rclcpp::get_logger("rgbd_synchronizer"), "Received IR camera info");
  copyCameraInfo(*msg, ir_info_msg);
  irinfo_sub.reset();  // Shutdown
}

void syncCallback(const Image::ConstSharedPtr &color,
                  const Image::ConstSharedPtr &depth,
                  const Image::ConstSharedPtr &thermal,
                  const Image::ConstSharedPtr &ir)
{
...
int main(int argc, char **argv)
{
  ...
  dinfo_sub = node->create_subscription<CameraInfo>(
    "/camera/depth/camera_info", 1, depthInfoCallback);
  irinfo_sub = node->create_subscription<CameraInfo>(
    "/camera/ir/camera_info", 1, irInfoCallback);
  ...
  ir_pub = it.advertise("/camera_throttled/ir/image_raw", 1);
  ...
  ir_info_pub = node->create_publisher<CameraInfo>("/camera_throttled/ir/camera_info", 1);
  ...
  static message_filters::Subscriber<Image> thermal_sub(node, "/camera/thermal/image_raw");
  static message_filters::Subscriber<Image> ir_sub(node, "/camera/ir/image_raw");

  static message_filters::Synchronizer<MySyncPolicy> sync(MySyncPolicy(10), color_sub, depth_sub, thermal_sub, ir_sub);
