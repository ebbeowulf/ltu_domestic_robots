...
class multi_query_localize(Node):
    ...
    def rgbd_callback(self, rgb_img:Image, depth_img:Image):
        ...
        if 1: # if the /map transform is correctly setup, then use tf all the way
            ...
            try:
                transform = self.tf_buffer.lookup_transform('base_link', depth_img.header.frame_id, Time.from_msg(depth_img.header.stamp))  
                trans = [transform.transform.translation.x, transform.transform.translation.y, transform.transform.translation.z]
                rot = [transform.transform.rotation.x, transform.transform.rotation.y, transform.transform.rotation.z, transform.transform.rotation.w]  
                ...
                #base_relativeM=np.matmul(tf_transformations.translation_matrix(trans), tf_transformations.quaternion_matrix(rot)) 
            except (LookupException, ConnectivityException, ExtrapolationException) as e:
                print("No Transform found")
                return
